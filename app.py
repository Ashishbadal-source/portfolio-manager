...
#         "V", "JNJ", "WMT", "PG", "MA", "UNH", "HD", "BAC", "XOM", "KO",
#         "PEP", "NFLX", "AMD", "INTC", "UBER", "PYPL", "SBUX", "IBM",
#         "ORCL", "QCOM", "INTU", "SHOP", "SPOT", "SQ", "SNAP",
#         "TCS.NS", "INFY.NS", "RELIANCE.NS", "HCLTECH.NS", "WIPRO.NS",
#         "ICICIBANK.NS", "HDFCBANK.NS", "SBIN.NS", "AXISBANK.NS",
#         "BAJFINANCE.NS", "KOTAKBANK.NS", "LT.NS", "MARUTI.NS",
#         "TATAMOTORS.NS", "TATASTEEL.NS", "SUNPHARMA.NS", "TITAN.NS",
#         "ADANIENT.NS", "DRREDDY.NS", "CIPLA.NS", "ULTRACEMCO.NS",
#     ]
...
<style>
  ...
  * { color-scheme: dark !important; }
  ...
  /* Boost base text contrast */
  p, span, div, li { color: #CBD5E1; }
  h1, h2, h3, h4, h5 { color: #F1F5F9 !important; font-weight: 700 !important; letter-spacing: -0.02em; }

  header[data-testid="stHeader"] { background-color: #060A14 !important; }
  footer { visibility: hidden; }
  ...
  /* Input labels — brighter & uppercase */
  ...
  /* ── Alerts ── */
  .stAlert p { color: inherit !important; font-size: 0.85rem !important; }
  ...
  .kpi-icon  { font-size: 1.25rem; margin-bottom: 10px; }
  .kpi-label {
      color: #64748B;
      font-size: 0.65rem;
      font-weight: 800;
  ...
  .kpi-delta-pos { color: #10B981; font-size: 0.76rem; font-weight: 700; margin-top: 4px; }
  .kpi-delta-neg { color: #F43F5E; font-size: 0.76rem; font-weight: 700; margin-top: 4px; }
  ...
  .news-title { color: #CBD5E1; font-size: 0.88rem; font-weight: 600; line-height: 1.5; }
  .news-meta  { color: #475569; font-size: 0.72rem; margin-top: 5px; }
...
with st.sidebar:
    ...
    <div class="user-badge">
        ...
        <div>
            ...
            <div class="user-status"><span class="status-dot"></span> Live session</div>
        </div>
    </div>""", unsafe_allow_html=True)
    ...
    st.markdown('<span class="sidebar-section">Add Stock</span>', unsafe_allow_html=True)
    ...
    ticker_input  = st.selectbox("Ticker Symbol", options=[""] + POPULAR_STOCKS,
                                  index=0, placeholder="Search or select...").upper().strip()
    buy_price_inp = st.number_input("Buy Price (USD)", min_value=0.01, value=100.0, step=0.01)
    quantity_inp  = st.number_input("Quantity", min_value=1, value=10, step=1)
    date_inp      = st.date_input("Purchase Date", value=date.today())
    ...
    st.markdown('<span class="sidebar-section">Remove Stock</span>', unsafe_allow_html=True)
    ...
    st.markdown('<span class="sidebar-section">Watchlist</span>', unsafe_allow_html=True)

    watch_ticker = st.text_input("Add Ticker", placeholder="e.g. GOOGL").upper().strip()
    if st.button("Add to Watchlist", use_container_width=True):
        if watch_ticker:
            add_to_watchlist(user_id, watch_ticker)
            st.success(f"{watch_ticker} added!")
    ...
    if not watchlist_df.empty:
        for _, wrow in watchlist_df.iterrows():
            wc1, wc2 = st.columns([3, 1])
            with wc1: st.write(wrow['ticker'])
            with wc2:
                if st.button("✕", key=f"wl_{wrow['id']}"):
                    remove_from_watchlist(user_id, wrow['ticker'])
                    st.rerun()

    st.markdown('<span class="sidebar-section">Settings</span>', unsafe_allow_html=True)
...
# ── Lazy data load — show header immediately, load data in background ──────────
raw_df = get_portfolio(user_id)

st.markdown(f"""
<div class="page-title">Portfolio Dashboard</div>
<div class="page-meta">{datetime.now().strftime('%A, %d %B %Y')} &nbsp;·&nbsp; Welcome, <b style="color:#94A3B8">{username}</b></div>
""", unsafe_allow_html=True)
st.markdown("---")
...
# Load prices only once per session, show a slim top-of-page progress bar
if not st.session_state.prices_loaded or st.session_state.enriched_df.empty:
    summary_df   = get_portfolio_summary(user_id)
    tickers_list = summary_df['ticker'].tolist()

    # Show a lightweight status — not a full-page spinner
    ...
    # Fallback to buy price if live price missing
    for ticker in tickers_list:
        if ticker not in prices or prices[ticker] is None:
            row = summary_df[summary_df['ticker'] == ticker]
            if not row.empty:
    ...
    status_placeholder.empty()   # remove the info bar immediately

enriched_df = st.session_state.enriched_df

if enriched_df.empty:
    st.info("No enriched data. Try refreshing prices.")
    st.stop()
...
with c2:
    dc = 'kpi-delta-pos' if delta_val >= 0 else 'kpi-delta-neg'
    ds = '+' if delta_val >= 0 else ''
    st.markdown(f"""
    <div class="kpi-card">
        <div class="kpi-icon">📈</div>
        <div class="kpi-label">Portfolio Value</div>
        <div class="kpi-value">${total_value:,.0f}</div>
        <div class="{dc}">{ds}${delta_val:,.0f}</div>
    </div>""", unsafe_allow_html=True)

with c3:
    pc = 'kpi-delta-pos' if total_profit >= 0 else 'kpi-delta-neg'
    ps = '+' if total_profit >= 0 else ''
    st.markdown(f"""
    <div class="kpi-card">
        <div class="kpi-icon">💰</div>
        <div class="kpi-label">Total P&L</div>
        <div class="kpi-value {pc}">{ps}${total_profit:,.0f}</div>
    </div>""", unsafe_allow_html=True)

with c4:
    rc = 'kpi-delta-pos' if total_return >= 0 else 'kpi-delta-neg'
    rs = '+' if total_return >= 0 else ''
    st.markdown(f"""
    <div class="kpi-card">
        <div class="kpi-icon">📊</div>
        <div class="kpi-label">Total Return</div>
        <div class="kpi-value {rc}">{rs}{total_return:.2f}%</div>
    </div>""", unsafe_allow_html=True)
...
with c5:
    ...
    <div class="kpi-card">
        <div class="kpi-icon">🏆</div>
        <div class="kpi-label">Best Stock</div>
        <div class="kpi-value" style="color:#10B981;font-size:1.15rem">{summary.get('best_stock','-')}</div>
    </div>""", unsafe_allow_html=True)
...
with c6:
    ...
    <div class="kpi-card">
        <div class="kpi-icon">📉</div>
        <div class="kpi-label">Worst Stock</div>
        <div class="kpi-value" style="color:#F43F5E;font-size:1.15rem">{summary.get('worst_stock','-')}</div>
    </div>""", unsafe_allow_html=True)
...
if not alerts_df.empty and not valid_df.empty:
    for _, alert in alerts_df.iterrows():
        ...
        target  = float(alert['target_price'])
        if alert['alert_type'] == 'above' and current >= target:
            st.success(f"🔔 {alert['ticker']} hit ${current:.2f} — above target ${target:.2f}")
        elif alert['alert_type'] == 'below' and current <= target:
            st.warning(f"🔔 {alert['ticker']} hit ${current:.2f} — below target ${target:.2f}")
...
with tab1:
    ...
    st.plotly_chart(portfolio_value_chart(tickers, quantities, buy_prices, hist_period),
                    use_container_width=True, key='portval')
    st.markdown('<div class="section-header">Portfolio Drawdown</div>', unsafe_allow_html=True)
    st.plotly_chart(portfolio_drawdown_chart(tickers, quantities, hist_period),
                    use_container_width=True, key='drawdown')
...
with tab2:
    st.markdown('<div class="section-header">Portfolio Allocation & P&L</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1: st.plotly_chart(allocation_pie(valid_df), use_container_width=True, key='pie')
    with c2: st.plotly_chart(profit_loss_bar(valid_df), use_container_width=True, key='plbar')
    st.markdown('<div class="section-header">Returns & Investment Comparison</div>', unsafe_allow_html=True)
    st.plotly_chart(return_pct_bar(valid_df), use_container_width=True, key='retbar')
    st.pyplot(invested_vs_current_matplotlib(valid_df))
    st.markdown('<div class="section-header">Stock Performance Comparison</div>', unsafe_allow_html=True)
...
with tab4:
    ...
    if len(tickers) >= 2:
        ...
        st.info("Add at least 2 stocks to see correlations.")
...
with tab5:
    ...
    if len(tickers) >= 2:
        ...
        st.info("Add at least 2 stocks to run optimization.")
...
with tab6:
    ...
    with sim_tab1:
        ...
        rw_days   = st.slider("Simulation Days", 30, 180, 90)
        rw_sims   = st.slider("Simulations", 50, 500, 100)
        if st.button("Run Simulation", key='rw_btn', type="primary"):
    ...
    with sim_tab2:
        ...
        with st.spinner("Running stress test..."):
            stress_df = run_stress_test(valid_df, custom_pct, custom_name if custom_name else None)
        if not stress_df.empty:
            st.plotly_chart(stress_test_chart(stress_df), use_container_width=True, key='stress')
            st.dataframe(stress_df, use_container_width=True, hide_index=True)
...
with tab7:
    ...
    with da1:
        ...
        with st.spinner("Computing statistics..."):
            ret_df = get_returns_matrix(tickers, hist_period)
        if not ret_df.empty:
            st.dataframe(get_descriptive_stats(ret_df), use_container_width=True)

    with da2:
        st.markdown('<div class="section-header">Data Quality Report</div>', unsafe_allow_html=True)
        st.caption("Missing values, duplicates, dtypes")
        from utils.analytics import get_data_quality_report
        raw_port = get_portfolio(user_id)
        if not raw_port.empty:
            report = get_data_quality_report(valid_df, raw_port)
    ...
    with da4:
        ...
        roll_ticker = st.selectbox("Select Stock", tickers, key='roll_sel')
        roll_window = st.slider("Rolling Window (days)", 10, 90, 30)
        with st.spinner("Computing rolling stats..."):
            ret_df = get_returns_matrix(tickers, hist_period)
        if not ret_df.empty:
            st.plotly_chart(rolling_stats_chart(ret_df, roll_ticker, roll_window),
                            use_container_width=True, key='rollstats')
        if len(tickers) >= 2:
            st.plotly_chart(rolling_correlation_chart(ret_df, roll_window),
                            use_container_width=True, key='rollcorr')
...
with tab8:
    ...
    with ins1:
        ...
        <div class="health-card">
            ...
            <div style="font-size:2.6rem;font-weight:800;color:{color};letter-spacing:-0.04em;font-family:'DM Sans',sans-serif">
                {score}<span style="font-size:1.1rem;color:#475569;font-weight:500">/100</span>
            </div>
            <div style="color:#64748B;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;margin-top:6px">
        ...
        for ticker in tickers:
            try:
                ...
                name = ticker; sector = pe = beta = mc = low52 = high52 = None
            with st.expander(f"{ticker} — {name or ticker}"):
                i1, i2, i3, i4, i5 = st.columns(5)
                ...
                <div class="news-card">
                    <div class="news-title">
                        <a href="{item['link']}" target="_blank"
                           style="color:#CBD5E1;text-decoration:none;">{item['title']}</a>
                    </div>
                    <div class="news-meta">{item['publisher']} &nbsp;·&nbsp; {item['time']}</div>
                </div>""", unsafe_allow_html=True)
        else:
        ...
        if not alerts_df.empty:
            ...
            for _, alert in alerts_df.iterrows():
                ...
                with ac1:
                    st.markdown(f"""
                    <div class="alert-card">
                        <b style="color:#E2E8F0">{alert['ticker']}</b>
                        <span style="color:#64748B"> — {alert['alert_type'].upper()} </span>
                        <b style="color:#818CF8">${alert['target_price']:.2f}</b>
                    </div>""", unsafe_allow_html=True)
                with ac2:
                    if st.button("Remove", key=f"del_{alert['id']}"):
                        delete_price_alert(user_id, alert['id'])
...
with tab9:
    ...
    with tax_tab:
        ...
        if not tax_df.empty:
            ...
            st.markdown(f"""
            <div class="tax-card">
                Short Term Tax ({short_rate}%): <b style="color:#F43F5E">${tax_summary['total_short_tax']:,.2f}</b><br>
                Long Term Tax ({long_rate}%): <b style="color:#F59E0B">${tax_summary['total_long_tax']:,.2f}</b>
                <hr style="border:none;border-top:1px solid rgba(255,255,255,0.07);margin:10px 0">
                Total Tax: <b style="color:#F43F5E">${tax_summary['total_tax']:,.2f}</b><br>
                Net After Tax: <b style="color:#10B981">${tax_summary['net_after_tax']:,.2f}</b>
            </div>""", unsafe_allow_html=True)
    ...
    with rebal_tab:
        ...
        if not rebal_df.empty:
            for _, row in rebal_df.iterrows():
                action = row['action']
                color  = '#10B981' if action=='Buy' else '#F43F5E' if action=='Sell' else '#64748B'
                ...
                <div class="alert-card" style="border-left-color:{color}">
                    ...
                    {row['shares_to_trade']} shares (${abs(row['diff_value']):,.2f})
                    &nbsp;|&nbsp;
                    <span style="color:#64748B">{row['current_weight']}% → {row['target_weight']}%</span>
                </div>""", unsafe_allow_html=True)
            st.dataframe(rebal_df, use_container_width=True, hide_index=True)
...
st.markdown("---")
st.markdown(
    "<div style='text-align:center;color:#1E293B;font-size:0.7rem;letter-spacing:0.06em;'>"
    "PORTFOLIO MANAGER &nbsp;·&nbsp; STREAMLIT &nbsp;·&nbsp; SUPABASE &nbsp;·&nbsp; YFINANCE &nbsp;·&nbsp; PLOTLY"
    "</div>",
    unsafe_allow_html=True
)